...
RECOVERY_OBJECT <- "data/objects/recovery_csf1r.rds"
if(file.exists(RECOVERY_OBJECT)) {
  figure_object <- join_rna(readRDS(RECOVERY_OBJECT),normalise=TRUE)
} else {
  warning("Reprocessing the independent recovery study; validate final clusters against the source figure.")
...
# Record the command log required by DoubletFinder's RNA branch, retaining the
# original shared feature set for PCA after the settings have been recorded.
...
p_umap <- ggplot(umap_data, aes(UMAP_1, UMAP_2, fill = cluster)) +
  ...
  coord_equal() +
  labs(x = "UMAP_1", y = "UMAP_2", fill = "Cluster") +
...
saveRDS(figure_object,file.path(OUTPUT_DIR,"recovery_csf1r.rds"),compress="xz")
export_cell_manifest(figure_object,file.path(OUTPUT_DIR,"recovery_cell_manifest.csv"))
